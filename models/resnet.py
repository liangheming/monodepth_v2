...
class ResNet(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv2d(3 * input_images, self.inplanes, kernel_size=7, stride=2, padding=3, bias=False)
        ...
        self.out_channels = [64, 64 * block.expansion, 128 * block.expansion,
...
def _resnet(
    ...
    if weights is not None:
        ...
        input_images = model.conv1.in_channels // 3
        if input_images > 1:
            conv_weight = torch.cat([deepcopy(weights_dict['conv1.weight']) for _ in range(input_images)], dim=1)
            weights_dict['conv1.weight'] = conv_weight
